...
Request::Request(sdbus::IConnection& connection, std::string path, CloseHandler closeHandler)
  ...
  m_impl->object
      ->addVTable(sdbus::registerMethod("Close").implementedAs([this]() { m_impl->close(); }))
...
void Request::setCloseHandler(CloseHandler closeHandler) {
  m_impl->closeHandler = std::move(closeHandler);
}
...
bool ProcessResult::commandNotFound() const {
  return exited && exitStatus == 127;
}
...
std::shared_ptr<AsyncProcess> AsyncProcess::start(Loop& loop, std::string command, std::string input, Callback callback) {
  std::signal(SIGPIPE, SIG_IGN);
  auto process = std::shared_ptr<AsyncProcess>(new AsyncProcess(loop, std::move(command), std::move(input), std::move(callback)));
