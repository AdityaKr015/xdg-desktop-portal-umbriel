...
std::optional<std::string> savePng(WaylandContext::ScreenshotResult& shot) {
  ...
  cairo_surface_t* surface = cairo_image_surface_create_for_data(data, cairoFormat, static_cast<int>(shot.width),
                                                                 static_cast<int>(shot.height),
                                                                 static_cast<int>(shot.stride));
  if (cairo_surface_status(surface) != CAIRO_STATUS_SUCCESS) {
    std::fprintf(stderr, "screenshot: cairo surface creation failed: %s\n",
  ...
  if (status != CAIRO_STATUS_SUCCESS) {
    std::fprintf(stderr, "screenshot: cairo PNG write failed for %s: %s\n", path.c_str(),
...
std::optional<Color> sampleColor(const WaylandContext::ScreenshotResult& shot, const OutputInfo& output, int32_t globalX,
                                 int32_t globalY) {
  ...
  const int32_t px = std::clamp(static_cast<int32_t>(std::floor(localX)), int32_t{0}, static_cast<int32_t>(shot.width - 1));
  const int32_t py = std::clamp(static_cast<int32_t>(std::floor(localY)), int32_t{0}, static_cast<int32_t>(shot.height - 1));
...
struct ScreenshotPortal::Impl {
  ...
  Impl(Loop& loop, sdbus::IConnection& connection, sdbus::IObject& object, const Config& config,
       WaylandContext& wayland)
      : loop(loop), connection(connection), object(object), config(config), wayland(wayland) {
    object
        .addVTable(sdbus::registerMethod("Screenshot")
                       .implementedAs([this](PortalResponse&& result, const sdbus::ObjectPath& handle,
                                             const std::string& appId, const std::string& parentWindow,
  ...
  struct Operation : public std::enable_shared_from_this<Operation> {
    ...
    void onCaptureConstraints(CaptureState& cap, uint32_t failureResponse, Callback onComplete) {
      ...
      cap.frame = portal.wayland.captureFrame(
          *cap.session, cap.buffer,
          /*onReady=*/
          [weakSelf, format, stride, onComplete = std::move(onComplete)](
              CaptureBuffer& /*buf*/, uint64_t /*ptsSec*/, uint32_t /*ptsNsec*/) {
            auto self = weakSelf.lock();
            if (!self || self->done || !self->capture) {
              return;
            }
          ...
          /*onFailed=*/
          [weakSelf, failureResponse](bool /*constraintsChanged*/) {
            auto self = weakSelf.lock();
            if (!self || self->done) {
              return;
            }
    ...
    void captureOutput(const std::string& outputName, uint32_t failureResponse) {
      ...
      startCapture(outputName, failureResponse,
  ...
  struct ScreenshotOperation : public Operation {
    ...
    void runChooser() {
      ...
      input.push_back('\n');
      process = AsyncProcess::start(portal.loop, portal.config.screencast.chooserCmd, std::move(input),
                                    [self](ProcessResult child) {
                                      if (self->done) {
                                        return;
                                      }
                                      if (child.commandNotFound()) {
                                        std::fprintf(stderr,
                                                     "screenshot: chooser command not found (%s); falling back to first output\n",
  ...
  struct PickColorOperation : public Operation {
    ...
    void runExternal() {
      ...
      process = AsyncProcess::start(portal.loop, portal.config.screenshot.colorPickCmd, {}, [self](ProcessResult child) {
        if (self->done) {
          return;
        }
    ...
    void captureAndSample(const PickPoint& point) {
      ...
      auto self = std::static_pointer_cast<PickColorOperation>(shared_from_this());
      startCapture(outputCopy.name, 1,
                    [self, outputCopy, px = point.x, py = point.y](
                        std::shared_ptr<Operation> /*base*/, WaylandContext::ScreenshotResult shot) {
...
ScreenshotPortal::ScreenshotPortal(Loop& loop, sdbus::IConnection& connection, sdbus::IObject& object,
                                   const Config& config, WaylandContext& wayland)
    : m_impl(std::make_unique<Impl>(loop, connection, object, config, wayland)) {}

ScreenshotPortal::~ScreenshotPortal() = default;

void ScreenshotPortal::onConfigChanged(const Config&, const Config& newCfg) {
