...
public:
  ...
  void setSelectionOptions(uint32_t sourceTypes, bool multiple, uint32_t cursorMode, uint32_t persistMode,
  ...
  bool addStream(Loop& loop, WaylandContext& wayland, std::unique_ptr<WaylandContext::CaptureSession> capture,
                 std::unique_ptr<PipeWireStream> stream, const CaptureConstraints& constraints,
